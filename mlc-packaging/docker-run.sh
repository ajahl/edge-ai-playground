...
IMAGE_TAG="${IMAGE_TAG:-mlc-packaging}"
...
FORCE_RECONVERT_ARG=()
if [[ -n "${FORCE_RECONVERT:-}" ]]; then
  FORCE_RECONVERT_ARG=(-e "FORCE_RECONVERT=${FORCE_RECONVERT}")
fi
...
if [[ "${#FORCE_RECONVERT_ARG[@]}" -gt 0 ]]; then
  docker_args+=("${FORCE_RECONVERT_ARG[@]}")
fi
...
docker_args+=(
  ...
  "${IMAGE_TAG}"
  "source /opt/conda/etc/profile.d/conda.sh && conda activate mlc && if [ -d /opt/mlc-llm-local ]; then export MLC_LLM_SOURCE_DIR=/opt/mlc-llm-local; git config --global --add safe.directory /opt/mlc-llm-local; python -m pip install /opt/mlc-llm-local --no-build-isolation; fi && cd /repo-root/mlc-packaging && bash ./run.sh"
)
