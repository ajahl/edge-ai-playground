...
The key difference from `gemma4-mlc-packaging` is that the model-specific values are configurable through `config.env`.
...
```bash
IMAGE_TAG=my-custom-mlc-packaging ./docker-build.sh
...
docker run --rm -it \
  ...
  -v "$HOME/custom-mlc-packaging:/host-workspace" \
  custom-mlc-packaging \
